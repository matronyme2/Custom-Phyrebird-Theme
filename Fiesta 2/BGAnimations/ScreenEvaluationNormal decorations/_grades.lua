...
t[#t+1] = LoadActor(THEME:GetPathS("","Sounds/NEW_RECORD.mp3"))..{
	OnCommand=function(self)
		local play_new_record = false; -- If a player has a new record and did not stage break, play the sound
		-- Check if player 1 has a new record
		if SCREENMAN:GetTopScreen():PlayerHasNewRecord(PLAYER_1) or 
		SCREENMAN:GetTopScreen():PlayerHasNewMachineRecord(PLAYER_1)
		then
			-- Check if player 1 did not stage break
			if not stage_break[PLAYER_1] then
				play_new_record = true;
			end
		end;
		-- Check if player 2 has a new record
		if SCREENMAN:GetTopScreen():PlayerHasNewRecord(PLAYER_2) or
		SCREENMAN:GetTopScreen():PlayerHasNewMachineRecord(PLAYER_2)
		then
			-- Check if player 2 did not stage break
			if not stage_break[PLAYER_2] then
				play_new_record = true;
			end
		end;
		-- If any player has a new record, play the sound
		if play_new_record then
